...
import { addToCart, getFavorites, toggleFavorite } from '#/lib/shop'
import type { catalog } from '#/lib/shop'

export function NftData({ item }: { item: (typeof catalog)[number] }) {
  const [quantity, setQuantity] = useState(1)
  const [isFavorite, setIsFavorite] = useState(() => getFavorites().includes(item.name))
  ...
  return (
    <section className="w-full max-w-[50%]">
      ...
      <p className="text-sm font-normal text-[#CFB28C] leading-6 mb-[13.38px]">
        {item.name} é um colecionável digital da coleção Kurio Editions,
      ...
      <div className="flex justify-between items-center mb-[13.38px]">
        <div className="flex gap-3 items-center w-full">
          <Button aria-label="Diminuir quantidade" onClick={() => setQuantity(current => Math.max(1, current - 1))} className="w-8.25 h-[49.5px] flex justify-center items-center rounded-[33px] bg-[#D28A4C]">
            <img src={Minus} alt="minus icon" />
          </Button>
          <span className="text-[20px] leading-7 font-normal text-[#F5F1EB]">
            {quantity}
          </span>
          <Button aria-label="Aumentar quantidade" onClick={() => setQuantity(current => current + 1)} className="w-8.25 h-[49.5px] flex justify-center items-center rounded-[33px] bg-[#D28A4C]">
            <img src={Plus} alt="minus icon" />
          </Button>
        </div>

        <div className="flex items-center gap-2 w-full">
          <Button onClick={() => { addToCart(item.name, quantity); window.location.assign('/pagamento') }} className="h-10 w-full max-w-32.5 flex items-center justify-center rounded-md bg-[#D28A4C] font-bold text-sm leading-5 text-[#140D0A]">
            Comprar
          </Button>
          <Button onClick={() => setIsFavorite(toggleFavorite(item.name))} aria-pressed={isFavorite} className="h-10 w-full max-w-32.5 flex items-center gap-2 justify-center rounded-md border border-[#D28A4C] font-medium text-sm leading-5 text-[#E89B55]">
            <span>
